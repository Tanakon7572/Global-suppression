...
<div class="container py-4">
    ...
    <div class="row">
            ...
            <div class="section-box">
                ...
                <div id="manualArea">
                    <div class="input-group mb-2">
                        <input type="email" class="form-control row-email" placeholder="example@mail.com" oninput="checkInputModes()">
        ...
        <div class="col-md-6">
            <div class="section-box">
                ...
                <input type="file" id="csvFile" class="form-control mb-2" accept=".csv" onchange="checkInputModes()">
    ...
    <div class="text-center my-3">
        <button id="btnExecute" onclick="startProcess()" class="btn btn-success btn-lg px-5 shadow fw-bold me-2">เริ่มส่งข้อมูล (Direct API Call)</button>
        <button onclick="clearAllInputs()" class="btn btn-secondary btn-lg shadow">รีเซ็ตข้อมูล</button>
    </div>
...
function checkInputModes() {
    const csvFile = document.getElementById('csvFile');
    const emails = document.querySelectorAll('.row-email');
    const addRowBtn = document.querySelector('button[onclick="addRow()"]');
    
    let hasEmailText = false;
    emails.forEach(input => {
        if (input.value.trim() !== '') hasEmailText = true;
    });
    
    let hasFile = csvFile.value !== '';
    
    // Disable logic
    if (hasEmailText) {
        csvFile.disabled = true;
    } else if (hasFile) {
        emails.forEach(input => input.disabled = true);
        if (addRowBtn) addRowBtn.disabled = true;
    } else {
        csvFile.disabled = false;
        emails.forEach(input => input.disabled = false);
...
function addRow() {
    ...
    div.innerHTML = `<input type="email" class="form-control row-email" oninput="checkInputModes()"><select class="form-select row-case" style="max-width: 110px;"><option value="unsub">Unsub</option><option value="bounce">Bounce</option><option value="spam">Spam</option></select><button class="btn btn-outline-danger" onclick="this.parentElement.remove(); checkInputModes();">✖</button>`;
